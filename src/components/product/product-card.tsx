...
export function ProductCard({ product, priority = false }: { product: Product; priority?: boolean }) {
  ...
  function selectColor(color: string) {
    ...
    setSelectedSize(getFirstAvailableSize(product, color));
  }
  ...
  return (
    <>
        ...
        <div className="flex flex-1 flex-col pt-3 sm:pt-4">
          ...
          <div className="mt-auto flex items-center justify-between gap-2 pt-3">
            ...
            <Button
              size="sm"
              variant="outline"
              className="h-9 shrink-0 border-gold/25 bg-ivory px-2 text-[10px] uppercase tracking-[0.08em] text-emerald hover:border-emerald hover:bg-emerald hover:text-white sm:px-3 sm:text-xs"
              onClick={openPicker}
      ...
      {pickerOpen && (
        <div className="fixed inset-0 z-[90] bg-black/40 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={`Choose options for ${product.name}`} onClick={() => setPickerOpen(false)}>
          <div className="flex min-h-full items-end justify-center sm:items-center sm:p-4">
            <div className="gold-edge max-h-[84dvh] w-full max-w-md overflow-y-auto rounded-t-[8px] border border-emerald/12 bg-white p-5 shadow-[0_24px_80px_rgba(4,45,40,0.22)] sm:rounded-[8px]" onClick={(event) => event.stopPropagation()}>
              <div className="flex items-start justify-between gap-4">
                <div className="flex min-w-0 gap-3">
                  <div className="relative h-20 w-16 shrink-0 overflow-hidden rounded-[6px] border border-gold/20 bg-ivory">
                    <Image src={product.images[0]} alt="" fill className="object-cover object-top" sizes="64px" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-gold">Choose options</p>
                    <h3 className="font-display mt-1 text-2xl font-semibold leading-tight text-emerald">{product.name}</h3>
                    <p className="mt-1 text-sm font-semibold text-emerald">{formatPrice(product.salePrice || product.price)}</p>
                  </div>
                </div>
                <button
                  type="button"
              ...
              <div className="mt-5 grid gap-4">
                ...
                <div className="grid gap-2 border-t border-emerald/10 pt-4 sm:grid-cols-[1fr_auto]">
